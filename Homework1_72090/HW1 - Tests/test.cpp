...
struct TestStore : ActionHandler 
{
	...
	{
		this->impl->setActionHandler(this);
	}
	...
	{
		this->impl->init(workerCount, startBanana, startSchweppes);
	}

	void advanceTo(int minute) 
	{
		this->impl->advanceTo(minute);
	}

	int getBanana() const 
	{
		return this->impl->getBanana();
	}

	int getSchweppes() const 
	{
		return this->impl->getSchweppes();
	}

	void addClients(const ClientList& clients) 
	{
		this->impl->addClients(clients.data(), clients.size());
	}

	void addClients(const Client& single) 
	{
		this->impl->addClients(&single, 1);
	}
